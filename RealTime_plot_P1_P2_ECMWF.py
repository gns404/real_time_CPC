...
def load_model_coeff(member, period, kind):
    path = COEFF_DIR / 'ECMWF' / f'ECMWF.P{period}.exWRGMM.{kind}.120mon.nc'
    return open_dataarray(path, kind).sel(ensemble=member)
...
def load_ecmwf_z500(init_date):
    path = RAW_DIR / init_date / 'z500' / f'ECMWF_{init_date}.nc'

    with xr.open_dataset(path) as ds:
        da = ds['z'].isel(number=slice(0, 25)).squeeze(
            ['forecast_reference_time', 'pressure_level'], drop=True
        )
        ...
        da = (da / GRAVITY).resample(time='1D').mean().load()

    return da.transpose('ensemble', 'time', 'lat', 'lon')


def load_ecmwf_tp(init_date):
    path = RAW_DIR / init_date / 'tp' / f'ECMWF_{init_date}.nc'

    with xr.open_dataset(path) as ds:
        da = ds['tp'].isel(number=slice(0, 25)).squeeze(
            'forecast_reference_time', drop=True
        )
        valid_time = pd.to_datetime(ds.valid_time.values)
        da = da.rename({
            'number': 'ensemble',
        ...
        da = da.assign_coords(time=valid_time).sortby('lon').sortby('lat')
        da = da.sel(lat=slice(19, 51), lon=slice(229, 306))
        da = da * 1000.0
        da = da - da.shift(time=1, fill_value=0)
        da = da.clip(min=0)
        da = da.assign_coords(
            time=pd.to_datetime(da.time.values) - pd.Timedelta(days=1)
        ).load()
...
def regrid_tp_to_obs_grid(tp_daily, obs_grid):
    tp_daily = tp_daily.interp(
        lat=hist_r.latitude.values,
        lon=hist_r.longitude.values,
        method='linear',
    )
    tp_daily = tp_daily.rename(lat='latitude', lon='longitude')
    tp_daily = tp_daily.assign_coords(
        latitude=hist_r.latitude.values,
        longitude=hist_r.longitude.values,
    )
    return tp_daily.transpose('ensemble', 'time', 'latitude', 'longitude')
...
def compute_tercile_probabilities(period_precip, init_date):
...
def run_ecmwf(init_date, save=True):
    out_dir = OUTPUT_DIR / init_date / MODEL_NAME
    out_dir.mkdir(parents=True, exist_ok=True)

    z500_daily = load_ecmwf_z500(init_date)
    ...
    raw_tp_daily = load_ecmwf_tp(init_date)
...
def issued_label_from_init_date(init_date):
    return pd.Timestamp(init_date).strftime('%B %d, %Y')
...
def plot_period_wt_probability(wt_probability, init_date, output_dir):
    ...
    fig.suptitle(
        f'ECMWF P1–P2 Weather Regime Probability\nIssued: {issued_label}',
        fontsize=20, fontweight='bold', y=1.02,
    )
    plt.tight_layout(rect=[0, 0, 1, 0.84])

    out_png = output_dir / f'ECMWF_P1-P2_wt_probability_{init_date}.png'
    fig.savefig(out_png, dpi=300, bbox_inches='tight')
    plt.close(fig)
    print(f'Saved: {out_png.name}')

def plot_ecmwf(results, init_date, equal_chance_threshold=0.40):
...
def load_saved_ecmwf(init_date):
    out_dir = OUTPUT_DIR / init_date / MODEL_NAME
...
def main():
    ...
    if args.mode in ('all', 'calculate'):
        results = run_ecmwf(args.init_date, save=True)
    else:
        results = load_saved_ecmwf(args.init_date)

    if args.mode in ('all', 'plot'):
        plot_ecmwf(results, args.init_date)
