...
def get_local_grib(init_date, member, variable):
    """Return a GRIB file from a locally mirrored NOMADS directory tree."""
    init_month = pd.Timestamp(init_date)
    if init_month.day != 1:
        raise ValueError(f'--init-date must be the first day of a month: {init_date}')

    day = member[:2]
    hour = member[2:]
    cycle_date = f'{init_month:%Y%m}{day}'
    cycle = f'{cycle_date}{hour}'
    filename = f'{variable}.01.{cycle}.daily.grb2'
    path = (
        RAW_DIR / f'cfs.{cycle_date}' / hour /
        'time_grib_01' / filename
    )
    if not path.is_file():
        raise FileNotFoundError(f'Local CFSv2 GRIB file not found: {path}')
    return path

def load_cfsv2_z500(init_date):
    members = []
    for member in CFSV2_MEMBERS:
        path = get_local_grib(init_date, member, 'z500')
...
def load_cfsv2_tp(init_date):
    members = []
    for member in CFSV2_MEMBERS:
        path = get_local_grib(init_date, member, 'prate')
        with xr.open_dataset(
            path, engine='cfgrib', backend_kwargs={'indexpath': ''}
        ) as ds:
...
def plot_period_wt_probability(wt_probability, init_date, output_dir):
    ...
    fig.suptitle(
        f'CFSv2 P1–P2 Weather Regime Probability\nIssued: {issued_label}',
        fontsize=20, fontweight='bold', y=1.02,
    )
    plt.tight_layout(rect=[0, 0, 1, 0.84])

    out_png = output_dir / f'CFSv2_P1-P2_wt_probability_{init_date}.png'
